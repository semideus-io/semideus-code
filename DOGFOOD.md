...
- 2026-07-17 · **fallback-tier proof** (headless `-p`, local qwen2.5-coder:1.5b via Ollama) · json tier worked twice, grounded answers from real tool runs — but `/cost` printed `0 in → 0 out` (Ollama usage not mapped), and in xml mode the model ignored the protocol three ways, once *fabricating a tool result it never ran* · map compat usage through in providers; xml tier needs an XML-native model — both on the ledger
